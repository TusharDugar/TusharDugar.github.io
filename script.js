...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (serviceCards.length > 0) {
      gsap.from(serviceCards, {
          ...
          scrollTrigger: { trigger: ".services-card-grid", start: "top 80%", toggleActions: "play none none none", once: true }
      });
    ...
    // --- REFACTORED: 3D Portfolio Carousel (Single Face, No Flip) ---
    ...
    if (ring && galleryItems.length > 0) {
        ...
        const dimmedBrightness = parseFloat(style.getPropertyValue('--gallery-dimmed-brightness')) || 0.5;
        const activeBrightness = parseFloat(style.getPropertyValue('--gallery-active-brightness')) || 1.1;

        function getRadius() {
            return parseFloat(style.getPropertyValue('--card-carousel-radius'));
        }
        ...
        function updateBrightness(ringRotation) {
            galleryItems.forEach((item) => {
                ...
                const brightness = (angleDiff < 45) ? activeBrightness : dimmedBrightness;
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'ArrowRight') targetRotation = (nearestIndex + 1) * angleStep;
            else if (e.key === 'ArrowLeft') targetRotation = (nearestIndex - 1) * angleStep;
            else return;
            animateToRotation(targetRotation);
        });
