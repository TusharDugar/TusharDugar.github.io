// Removed GSAP and ScrollTrigger registration as they are no longer used.
...
function initScrollAnimations() {
  ...
  const observer = new IntersectionObserver((entries, observer) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        // Specific stagger logic for .reveal-parent (like about left content)
        ...
        // Specific stagger for .about-content-wrapper (like paragraphs/blockquote in about right)
        ...
        // Specific stagger for .services-items-container (for the 8 service cards)
            ...
            children.forEach((child, index) => {
                child.style.transitionDelay = `${index * 0.1s}`; // Use CSS nth-child if that's preferred, or this
                child.classList.add("visible");
            });
        }
  ...
  // Observe elements (including the services-items-container for stagger effect)
  revealElements.forEach(el => observer.observe(el));
  // Separately observe the services-items-container if it's not already covered
  const servicesContainer = document.querySelector('.services-items-container');
...
window.addEventListener("scroll", () => {
  ...
  sections.forEach(section => {
    const sectionTop = section.offsetTop - 150; // Offset for when section is considered "active"
